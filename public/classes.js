class Packet extends Phaser.GameObjects.GameObject {
  constructor(scene) {
    ...
    this.timeout = 3;
    this.expired = false;
    this.delayedCall = scene.time.delayedCall(
      this.timeout * 1000,
...
class Generator extends Machine {
  ...
  startGeneration() {
    this.interval = setInterval(this.createPacket.bind(this), this.trafficRate);
  }
  incriseTraffic() {
    clearInterval(this.interval);
    this.trafficRate = Math.floor(this.trafficRate / 2);
    this.interval = setInterval(this.createPacket.bind(this), this.trafficRate);
  }
}
